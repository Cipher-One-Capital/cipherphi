...
Four pillars, all of them already in the protocol rather than built for this project.

**Natively issued USDC.** Circle issues USDC on Stellar directly, and CCTP moves it in from other chains by burn and mint rather than through a wrapped asset or a third-party bridge.[^4] A pool holding donated zakat cannot carry bridge counterparty risk, and on Stellar it does not have to.

**A native token interface.** Every Stellar asset has a Stellar Asset Contract at a deterministic address implementing SEP-41, so custody and transfer are a standard interface call. There is no token contract in CipherPhi's codebase to write, audit, or get wrong.

**Cheap settlement, suited to per-category accounting.** Fees are metered in stroops rather than in dollars.[^5] Splitting one contribution across eight zakat categories and emitting an event for each is affordable, where on a chain with meaningful gas it would push the project toward batching, which is the thing that destroys the audit trail.
...
Soroban then supplies the part a payment network alone cannot do: the eight-category allocation and the eligibility gate run as enforced code rather than as application logic sitting above the rails.

  

# Architecture
*Three Soroban contracts, three separated keys, and a ledger that carries no personal data.*

## The boundary

Funds and rules live on Stellar. Identity and human judgment stay off it.

Every other decision in this section follows from that one line. Three things it buys, and one it costs.

**Erasure stops being a contradiction.** An immutable chain and a right to erasure are only in tension if personal data reaches the ledger. Nothing personal does, so the obligation attaches to the off-chain registry and is met there. This is the failure that ends a project of this shape after launch rather than before it.
...
**The cost, and how it is paid.** Eligibility becomes a claim made off-chain. That claim is signed, and the signature is verified onchain before any money moves. A reviewer does not have to trust the eligibility decision in order to verify that one was made, by a known key, before funds left the pool.
...
```
*Only three things cross the line: a signed transaction, a signed attestation, and reads of public ledger events. No identity, no document, no name.*

## Three contracts, split by what each may touch

Custody, policy, and eligibility are separate deployments from the first day, not modules inside one binary that someone promises to split later.
...
**The split line is value, not tidiness.** One contract holds assets. The other two are consulted by it and can never move anything. That is what makes the split worth its cost: the two components most likely to change, a refinement to the Shariah parameters and a change to the eligibility scheme, are exactly the two that hold no money. A defect in either is a wrong answer, not a loss.

**Upgrade becomes granular instead of total.** Refining the amil cap redeploys `policy`. Changing the attestation scheme redeploys `attestation`. Neither touches custody, so donor funds never migrate for a reason unrelated to custody. A single contract would have forced the opposite: every change to a Shariah parameter redeploying the thing holding the money.

**The cost is a trust boundary, and it is paid explicitly.** A cross-contract call is a place where a wrong answer can enter. `zakat_pool` pins the addresses of `policy` and `attestation` in governance-controlled storage, and treats both as untrusted inputs rather than trusted collaborators: an allocation is rejected unless the returned category amounts sum exactly to the amount received, and a distribution is rejected unless the attestation verifies against the authority the pool itself holds. The audit scope is three Wasm binaries rather than one, which is a real cost and is the reason independent review is entered well before mainnet rather than alongside it.
...
Storage is bounded by design, and the bound is the point.

Every Soroban entry carries a time to live, and a persistent entry whose TTL lapses is moved to an archive and must be restored before it can be read again.[^8] Storage types differ in cost, in how many keys a contract may hold, and in what happens at expiry.[^9] An architecture that writes one record per contribution therefore has a ceiling it will meet without warning, and meets it at exactly the moment the platform is working.

| Contract | Entry | Content |
| --- | --- | --- |
| `zakat_pool` | `governance` | Multisig address. Configuration and deployment authority |
| `zakat_pool` | `distributor` | The key permitted to move funds out of the pool |
| `zakat_pool` | `policy_addr` | Address of the policy contract, governance-controlled |
| `zakat_pool` | `attest_addr` | Address of the attestation contract, governance-controlled |
| `zakat_pool` | `approved_assets` | SAC addresses for USDC, and later EURC |
| `zakat_pool` | `category_balances` | Eight buckets per approved asset. The whole of the accounting state |
| `zakat_pool` | `distribution_seq` | Monotonic counter, bound into every attestation so a signature executes once |
| `zakat_pool` | `paused` | Contribution pause flag, governance-controlled |
| `zakat_pool` | `pending_change` | A queued governance change: what it targets, its new value, and the ledger after which it may execute |
| `policy` | `allocation` | The eight-category split, extendable to a per-partner split |
| `policy` | `parameters` | Nisab basis, amil cap, discharge timing |
| `attestation` | `authority` | Public key set that signs recipient eligibility. Signers, not fund-movers |

That is the complete persistent set across all three contracts. It grows only when governance approves an additional asset, which adds eight buckets. Nothing here grows with donor count, contribution count, or recipient count. A TTL-bump strategy runs on every entry above, so nothing load-bearing is archived through disuse.
...
Each contribution emits one event carrying the donor address, the asset, the amount, and the resulting eight-category split. Each distribution emits one carrying the recipient address, the category, the amount, and the hash of the attestation that authorised it. Those two event types are the complete record, and between them they reconstruct any view of the pool at any point in its history.
...
Two functions on `zakat_pool` move value. Everything else across the three contracts is configuration, and these two are what the audit is really about.

`contribute(asset, amount)`

1. Reject unless `asset` is in `approved_assets`
2. Pull the asset from the donor via the Stellar Asset Contract
3. Call `policy` for the eight-category split, and reject unless the returned amounts sum exactly to the amount received
4. Increment the category buckets with checked `i128` arithmetic
5. Emit a Contribution event carrying donor address, asset, amount, and the resulting split
...
1. `require_auth` on the distributor key
2. Call `attestation` to verify the signature, which binds recipient, asset, category, amount, and the pool's distribution sequence
3. Check the category bucket holds `amount`
4. Decrement the bucket, then transfer, in that order
5. Emit a Disbursement event carrying the recipient address, the category, and the attestation hash

**Ordering.** Step 4 is checks-effects-interactions. Soroban's execution model does not permit reentrancy, so this ordering is a backstop rather than the primary control. It is written this way regardless, because a contract whose safety depends on a platform guarantee reads as safe for the wrong reason, and because the pattern survives a future platform change that the assumption would not.
...
| --- | --- | --- | --- |
| Governance | 3-of-5 multisig | Set policy, approve assets, deploy, pause contributions, repoint `policy` and `attestation` under timelock | Move funds. Sign eligibility |
...
**No seat is shared.** The distributor key is held by the delivery team. No delivery-team seat sits in the attestation quorum, whose three seats are the Foundation's Executive Director, the Shariah authority, and an independent seat appointed by the Foundation. That exclusion is the point rather than an accident of staffing: with a seat, the delivery team plus any one other signer would produce a valid attestation for any address and execute it in the same transaction, instantly and with no timelock, which would be a strictly easier path than anything described below. Without a seat, a delivery team that wants to move funds needs an attestation it cannot produce, and an attestation quorum that wants to move funds holds no key that can call `distribute`.

The Shariah authority holds a seat because eligibility is partly a category question, whether a recipient falls inside one of the eight, which is the same judgment the parameter gate already covers.

**The honest limit.** What remains is collusion between the Foundation and the delivery team, through governance repointing `attestation` to a permissive contract. That is the floor rather than an oversight: two organisations, two key sets, and no way to reduce it further without a third party holding neither. Three things constrain it, all onchain. Repointing `attestation` or `policy`, and rotating the `distributor`, are each subject to a seven-day timelock that emits an event at proposal and at execution, so the change is visible before it takes effect rather than after. Approving an asset sits under the same delay; pausing contributions does not, because a brake that takes a week is not a brake. Governance can cancel a queued change, and the cancellation emits too. And the pilot runs with capped amounts, so the window between a visible proposal and a possible loss is bounded by design. Holder identities and custody method are published with the mainnet deployment.
...
**EURC is a second denomination, not a second integration, and the accounting is built for it from the start.** Category balances are keyed by asset, so each approved asset carries its own eight buckets and its own solvency invariant. The alternative, one set of buckets across two denominations, would be an accounting error rather than a shortcut: zakat owed in euros is not discharged by dollars at an implied rate the contract never agreed.
...
A donor sees their contribution move through four states, and each one is read from a ledger event that carries the transaction hash which produced it.
...
*The interface is a rendering of public data rather than a claim about private data.*
...
Before anyone can contribute, they need a Stellar account, a USDC trustline, and USDC. Privy supplies keys. It supplies none of those three, and the gap is worth stating plainly rather than leaving between the lines.
...
**How many such donors exist is measured, not asserted.** This document does not claim a quantified population of stablecoin-holding donors inside the Foundation's base, because no survey has been run. Running one is an early delivery activity, and its output is a written report of how many donors were contacted, how many hold or can acquire USDC, and how many committed to the pilot. If that number comes back too small to run a donor-side pilot, the honest consequence is that the pilot demonstrates the distribution path with Foundation-originated contributions and the donor-acquisition question moves to the phase that funds an on-ramp.

**A fiat on-ramp is a later phase, and it is a Stellar integration rather than a build.** Reaching a donor who holds only Swiss francs means a SEP-24 anchor converting fiat to USDC on Stellar, which is exactly the interface anchors exist to provide. It is named in the building blocks table as planned, and the trigger for commissioning it is demonstrated pilot demand from donors who cannot self-fund.
...
The platform holds recipient personal data, and none of it is written to the Stellar ledger. Not a name, not a document reference, not an identifier that resolves to a person. What the ledger holds is addresses, amounts, and attestation hashes.
...
| Category split | The distribution across the eight zakat categories |
...
**Access control, the costliest 2025 category at USD 953 million in losses.** Three separated roles behind `require_auth`: governance sets policy, a distributor moves funds, an attestation authority signs eligibility, and no single key does two of those. Governance is a multisig from deployment; the attestation authority is 2-of-3 before mainnet. The keys that can move money cannot change the rules, and the keys that set the rules cannot move money.
...
**Storage bounds.** Only configuration and the per-asset balance buckets are persistent across the three contracts, with a TTL-bump strategy on every entry, no unbounded maps, and no per-transaction iteration over a collection. Nothing any contract reads grows with usage, so read limits stay clear as the pool fills.
...
**Fuzzing.** `cargo-fuzz` runs on the fund-moving paths, targeting `contribute` and `distribute` specifically, since those are the two functions where a malformed input becomes a balance error. Fuzzing is first-class in the Stellar toolchain: the SDK ships `arbitrary` support for contract types, and the official guide covers cargo-fuzz, property tests, and mutation testing.[^20]
...
| Circle CCTP | Native inbound USDC from other chains, terminating before CipherPhi's code | Planned |
...
21. Standardised asset events under SEP-41  
